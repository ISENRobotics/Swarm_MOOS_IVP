...
OBSTACLE_POS1="{-90,-50:30,-110:-90,-110}"
OBSTACLE_POS2="{195,-50:75,-110:195,-110}"
...
for i in `seq 1 $NB_ROBOT`; do
	...
	if [ $pair_impair = "0" ] ; 
		...
		then
			let "positionX = i * 15 - 25"
			START_POS="$positionX,-50" 
			let "positionX = i * 15 - 25"
			STOP_POS="$positionX,-170" 
		
	    	else

			let "positionX = i * 15 - 25"
			START_POS="$positionX,-40"
			let "positionX = i * 15 - 25"
			STOP_POS="$positionX,-160"    
	fi
    ...
    START_POS=$START_POS STOP_POS=$STOP_POS OBSTACLE_POS1=$OBSTACLE_POS1 OBSTACLE_POS2=$OBSTACLE_POS2   
